...
const GroupsPage = () => {
  ...
  } = useFirebaseData(fetchAllGroups, [], {
    enabled: true,
    initialData: [],
  ...
  const filteredGroups = useCallback(() => {
    ...
    const filtered = displayedGroups.filter((group) => {
      ...
      const subjectMatch =
        ...
        selectedSubject === '' ||
        (group.subject &&
          Array.isArray(group.subject) &&
          group.subject.includes(selectedSubject));
  ...
  // 필터링된 그룹 데이터
  const displayGroups = isFilterAnimating
    ? searchResults
  ...
  return (
    <div className={`main-layout groups-page ${darkMode ? 'dark-mode' : ''}`}>
      ...
      <main className="main-content">
        <Container
          className={`groups-container ${darkMode ? 'dark-mode' : ''}`}
        >
          {/* 로딩 상태 */}
          {isLoading ? (
            ...
            <>
              ...
              {currentUser && userGroups && userGroups.length > 0 && (
                <>
                  <h3
                    className="mb-4"
                    style={{ marginTop: '2rem', fontSize: '2.2rem' }}
                  >
                    참가중인 그룹
                  </h3>
              ...
              {/* 검색 및 필터 */}
              <div
                className={`search-filter-card ${darkMode ? 'dark-mode' : ''} ${isFilterAnimating ? 'filter-animating' : ''}`}
              >
              ...
              {displayGroups.length > 0 ? (
                ...
                <div className="text-center my-5">
                  ...
                  {(searchTerm || selectedSubject || selectedTags.length > 0) &&
                    !isSearching && (
                      <Button
                        variant="outline-secondary"
                        onClick={() => {

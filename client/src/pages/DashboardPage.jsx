...
const DashboardPage = () => {
  ...
  // 사용자 이름 가져오기
  const displayName =
    userProfile?.displayName || currentUser?.email?.split('@')[0] || '사용자';
  ...
  const refreshData = useCallback(async () => {
    try {
      await Promise.all([refetchGroups(), refetchEvents()]);
    } catch (error) {
      // 데이터 새로고침 실패는 조용히 처리
    }
  ...
  return (
    <div
      className={`main-layout dashboard-page ${darkMode ? 'dark-mode' : ''}`}
    >
      {/* 메인 콘텐츠 영역 */}
      <main className="main-content dashboard-content">
        ...
        {(groupsError || eventsError) && (
          <div
            className={`alert alert-danger mt-3 ${darkMode ? 'dark-mode' : ''}`}
            role="alert"
          >
        ...
        <div className="dashboard-center-content mb-4">
          <div
            className={`welcome-message-component ${darkMode ? 'dark-mode' : ''}`}
          >
        ...
        <Container
          className={`dashboard-cards-container ${darkMode ? 'dark-mode' : ''}`}
        >
          <Row className="g-4 mb-4 ">
            ...
            <Col lg={showAdminCards ? 4 : 6} md={6} sm={12}>
              <UniversalCard
                ...
                headerClassName="card-header"
              >
                <GroupRecommendationComponent userGroups={safeUserGroups} />
              </UniversalCard>
            </Col>
          </Row>
          ...
          <Row className="g-4 mb-4">
            ...
            <Col lg={4} md={6} sm={12}>
              ...
              >
                <div
                  className={`mini-calendar-component ${darkMode ? 'dark-mode' : ''}`}
                >
            ...
            <Col lg={4} md={6} sm={12}>
              ...
              >
                {/* 여기부터 2x2 그리드 적용 */}
                <Row
                  className={`meeting-stats-component g-2 ${darkMode ? 'dark-mode' : ''}`}
                >
                  <Col xs={6}>
                    {' '}
                    {/* 작은 화면에서도 2x2 유지를 위해 col-6 적용 */}
                    <StatWidget
                      icon="bi-people-fill"
                      label="참여 그룹"
            ...
            <Col lg={4} md={12} sm={12}>
              ...
              >
                <div className="d-flex align-items-center justify-content-center">
                  <div
                    className={`timer-card-component ${darkMode ? 'dark-mode' : ''}`}
                  >
                    {/* 세션 카운터 */}
                    {sessions > 0 && (

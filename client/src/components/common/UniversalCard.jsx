...
const UniversalCard = ({
  ...
  const getCardClasses = () => {
    ...
    const variantClass =
      variant !== 'default' ? `universal-card-${variant}` : '';
  ...
  const renderHeader = () => {
    ...
    return (
      <div className={`universal-card-header ${headerClassName}`}>
        ...
        {headerAction && (
          <div className="card-header-actions">{headerAction}</div>
        )}
      </div>
    );
  };
  ...
  return (
    <div className={getCardClasses()} onClick={onClick} {...props}>
      {renderHeader()}
      <div className="universal-card-content">{children}</div>
    </div>
  );
};

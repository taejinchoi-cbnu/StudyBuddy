...
const AppNavbar = forwardRef((props, ref) => {
  // 컨텍스트 및 훅
  const { currentUser, logout, login, signup, resetPassword, authLoading } =
    useAuth();
  ...
  const handleSignup = async (e) => {
    ...
    if (!email.endsWith('@chungbuk.ac.kr')) {
      return showError(
        '충북대학교 이메일(@chungbuk.ac.kr)만 가입할 수 있습니다.',
      );
    }
    ...
    try {
      ...
      try {
        ...
        showSuccess(
          '회원가입이 완료되었습니다. 프로필에서 이메일 인증을 완료해주세요.',
        );
      ...
      >
        <Container>
          ...
          <Navbar.Brand
            as={Link}
            to="/"
            className="d-flex align-items-center navbar-brand-container"
          >
            <img
              src={logoSmall}
              alt="StudyBuddy Logo"
            ...
            <Nav className="me-auto nav-links-container">
              {currentUser && (
                <>
                  ...
                  <Nav.Link as={Link} to="/groups" className="nav-link-item">
                    그룹
                  </Nav.Link>
                  <Nav.Link as={Link} to="/schedule" className="nav-link-item">
                    일정
                  </Nav.Link>
                </>
              )}
        ...
        <Modal.Body>
          ...
          <Form onSubmit={handleResetPassword}>
            ...
            <p className="text-muted mt-3">
              가입하신 이메일 주소를 입력하시면 비밀번호 재설정 안내가
              발송됩니다.
            </p>
            <Button
              variant="primary"

...
const GroupScheduleComponent = ({ group, members, onGroupUpdate }) => {
    ...
    try {
      ...
      setAvailabilityData((prevData) => {
        ...
        if (!prevData[userId]) {
          prevData[userId] = {
            name:
              members.find((m) => m.userId === userId)?.displayName || '사용자',
            unavailableTimes: [],
          };
        }
        ...
        const updatedData = {
          ...
          [userId]: {
            ...prevData[userId],
            unavailableTimes: [...prevData[userId].unavailableTimes, newTime],
          },
        };
      ...
      DAYS_OF_WEEK.forEach((day) => {
        ...
        for (let i = 0; i < allTimeSlots.length; i++) {
          if (!unavailableBlocks.includes(i)) {
            ...
            if (
              i === allTimeSlots.length - 1 ||
              unavailableBlocks.includes(i + 1)
            ) {
              if (startBlock !== null) {
  ...
  const getMemberSubmissionStatus = () => {
    ...
    return members.map((member) => {
      const hasSubmitted =
        availabilityData[member.userId] &&
  ...
  const getSubmissionPercentage = () => {
    ...
    const submittedCount = status.filter(
  ...
  const handleAppointment = async (appointmentData, action = 'add') => {
    ...
    try {
      ...
      if (action === 'delete') {
        ...
        await deleteGroupAppointment(
          group.id,
          appointmentData.id,
          currentUser.uid,
        );
        setAppointments((prev) =>
          prev.filter((app) => app.id !== appointmentData.id),
        );
        setSuccess('일정이 삭제되었습니다.');
      } else {
        // 일정 추가 - userId 파라미터 추가
        const savedAppointment = await saveGroupAppointment(
          group.id,
          appointmentData,
          currentUser.uid,
        );
  ...
  if (!userStatus.isMember) {
    return (
      <div
        className={`schedule-component access-denied ${darkMode ? 'dark-mode' : ''}`}
      >
  ...
  return (
    <div className={`schedule-component ${darkMode ? 'dark-mode' : ''}`}>
      <div className="schedule-component-body">
        ...
        <div className="mb-3">
          <p>모든 그룹 멤버들의 가능한 시간을 찾아드립니다.</p> <br />
          <p>
            ...
            <div className="schedule-section-header">
              <div className="d-flex justify-content-between align-items-center">
                ...
                <button
                  ...
                  onClick={() =>
                    setShowConfirmedSchedules(!showConfirmedSchedules)
                  }
                >
                  ...
                  <Alert key={idx} variant="success" className="mb-2">
                    <div className="d-flex justify-content-between align-items-center">
                      <div>
                        ...
                        <p className="mb-0">
                          <strong>일시:</strong> {appointment.day},{' '}
                          {appointment.start} - {appointment.end}
                        </p>
                        <small className="text-muted">
                          {appointment.createdAt
                            ? `등록일: ${new Date(appointment.createdAt).toLocaleDateString()}`
                            : ''}
                        </small>
                      </div>
                      {userStatus.isAdmin && (
                        <button
                          ...
                          onClick={() =>
                            handleAppointment(appointment, 'delete')
                          }
                        >
                          삭제
                        </button>
          ...
          <div className="schedule-section-body">
            ...
            {currentUser && (
              <ScheduleManager
                ...
                existingTimes={
                  availabilityData[currentUser.uid]?.unavailableTimes || []
                }
                onRemove={(index) =>
                  removeUnavailableTime(currentUser.uid, index)
                }
          ...
          <div className="d-grid gap-2 mb-4">
            ...
            {!allMembersSubmitted() && (
              <small className="text-muted text-center mt-1">
                아직 미제출한 멤버가 있습니다. 모든 멤버가 입력 완료해야 계산이
                가능합니다.
              </small>
            )}
          </div>
